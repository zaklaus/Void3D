...
class C_vehicle_imp: public C_actor_physics{
   ...
   bool ContactReport(PI3D_frame dst_frm, const S_vector &pos, const S_vector &normal, float depth,
      bool play_sounds = true) override{
...
public:
   ...
   const C_table_template *GetTableTemplate() const override{ return &templ_veh_props; }
         ...
         if(!mission.LoadPhysicsTemplate(C_xstr("Models\\%\\scene.bin") % &mname[1], templ))
            ...
            for(dword i = joints.size(); i--;){
               ...
               if(jnt->GetType() == IPHJOINTTYPE_HINGE2){
                  ...
                  auto jh = static_cast<PIPH_joint_hinge2>(jnt);
   ...
   bool Use(E_USE_TYPE use_type /*= USE_TOGGLE*/, C_actor *instigator /*= NULL*/,
      PI3D_frame hit_frm /*= NULL*/) override{
   ...
   void Input(const S_tick_context &tc) override{
      float tsec = static_cast<float>(tc.time) * .001f;
      ...
      if(can_control){
         // orbit camera
         {
            float rx = static_cast<float>(tc.mouse_rel[0]), ry = static_cast<float>(tc.mouse_rel[1]);
            //mouse - apply sensitivity and speed
            if(tc.p_ctrl){
               if(tc.p_ctrl->GetConfigValue(C_controller::CFG_INVERT_MOUSE_Y))
                  ry = -ry;

               float f = static_cast<float>(tc.p_ctrl->GetConfigValue(C_controller::CFG_MOUSE_SENSITIVITY)) * .01f;
               rx *= f;
               ry *= f;
               ...
               if(cam){
                  auto rot = S_quat(S_vector(0, 1, 0), (mx) * (PI / 180.0f));
                  cam->SetDeltaRot(rot);
               }
            }
         }
   ...
   void Tick(const S_tick_context &tc) override{
...

      {
         ...
         if(!want_dir){
            ...
            vel = .5f + rel_speed * .3f;
            vel *= static_cast<float>(want_dir);
         }
         //process all wheels
...
PC_actor CreateVehicleActor(C_game_mission &gm1, PI3D_frame in_frm){
   auto vp = new C_vehicle_imp(gm1, in_frm);
   return vp;
}
